...
This fork is the live node runtime for the experimental `tap-ldk` Taproot Assets demo. It is currently pinned to `OpenAgentsInc/rust-lightning` revision `0a89b49bf1e822353e0e7c482c5630d5dff22c5c` with the simple-taproot MuSig2 feature enabled, and it exposes `ldk_node::provenance::runtime_provenance()` so demo harnesses can verify the forked runtime. BTC-only behavior remains the default node behavior. Regtest callers can opt into the experimental simple-taproot and Taproot Asset channel negotiation flags through `ExperimentalChannelConfig`; Taproot Asset channels fail closed unless simple-taproot channels are also enabled, simple-taproot/Taproot Asset opens are private-only, and missing required open/accept nonces fail closed. `Node::taproot_asset()` now exposes the experimental custom-message, asset-channel open, and asset-payment state APIs needed by `tap-ldk`, including live `AssetFundingCreated` parsing, Lightning Labs proof-type compatibility repair, proof-derived funding tapscript-root binding, proof-derived single-asset channel-template binding, bounded initial commitment aux-leaf binding, live pending-channel binding, claimed asset HTLC balance persistence, full counterparty commitment monitor persistence for Taproot Asset channels, live `commitment_signed` asset-signature blob preservation/validation, Lightning Labs commitment aux-leaf decoding, and exact previous-output-bound second-level HTLC aux leaves for the current one-asset, one-script-key live demo path. The fork advertises Lightning Labs no-op HTLC aux support but does not advertise STXO support until native STXO commitment leaves are implemented; live Lightning Labs settlement remains tracked in `OpenAgentsInc/tap-ldk` issue #81.
