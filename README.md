...
This fork is the live node runtime for the experimental `tap-ldk` Taproot Assets demo. It is pinned to `OpenAgentsInc/rust-lightning` revision `3db3229733b724f45e7a356d923715213cb4f269` with simple-taproot MuSig2 enabled, and `ldk_node::provenance::runtime_provenance()` reports that exact runtime. BTC-only behavior remains the default. Regtest callers can opt into experimental simple-taproot and Taproot Asset channel flags through `ExperimentalChannelConfig`; those channels stay private, require simple-taproot nonces, and fail closed if Taproot Assets are requested without the simple-taproot base. `Node::taproot_asset()` exposes the experimental custom-message, asset-channel open, and asset-payment APIs needed by `tap-ldk`, while the pinned LDK fork now replay-checks the BOLT simple-taproot commitment, HTLC, unilateral-spend, and restart metadata vectors. The fork advertises Lightning Labs no-op HTLC aux support but does not advertise STXO support until native STXO commitment leaves are implemented.
